...
public class JdbcReaderRepository implements ReaderRepository {
    ...
    @Override
    public int update(ReaderEntity reader, String newName) {
        try (Connection con = db.getConnection();
             PreparedStatement ps = con.prepareStatement(Queries.UpdateReader)) {
            ps.setString(1, newName);
            ps.setInt(2, reader.getId());
            int rowsAffected = ps.executeUpdate();
            if (rowsAffected == 0) {
                throw new DatabaseError(Constantes.DB_ERROR2 + reader.getId());
            }
            return rowsAffected;
        } catch (SQLException e) {
